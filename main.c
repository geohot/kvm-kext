...
static int kvm_open(dev_t Dev, int fFlags, int fDevType, struct proc *pProcess) {
  return -1;
}

static int kvm_close(dev_t Dev, int fFlags, int fDevType, struct proc *pProcess) {
  return -1;
}

static int kvm_ioctl(dev_t Dev, u_long iCmd, caddr_t pData, int fFlags, struct proc *pProcess) {
